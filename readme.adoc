...
[source,bash]
----
ansible-playbook -i localhost, bootstrap/ansible/bootstrap.yaml --extra-vars config=basic --extra-vars global_hub=yes --ask-vault-password
----
...
[source,bash]
----
ansible-playbook -i localhost, bootstrap/ansible/bootstrap.yaml --ask-vault-password \
  --extra-vars config=ha --extra-vars global_hub=no --extra-vars env=prod
----
...
. ArgoCD generates Helm manifests by running the `helm template` command rather than using `helm install`. The resulting manifests are then applied to the cluster using `kubectl apply`. This approach has two key implications:
** No Helm release metadata is stored in the cluster.
** Templates cannot query live Kubernetes resources during rendering. Specifically, the Helm `lookup` function is not supported when using ArgoCD. For more details, see: https://github.com/argoproj/argo-cd/issues/21745.
...
. When managing CPU resource requests and limits with ArgoCD, specify whole CPU cores as `1` or `2` instead of `1000m` or `2000m`. Kubernetes automatically normalizes these values (e.g., converting `1000m` to `1`), but ArgoCD often compares the raw Git manifest against the normalized live cluster state. This mismatch can cause a perpetual `OutOfSync` status and result in sync loops.
...
- Tips for using templating in RHACM governance policies:
  ** link:https://www.redhat.com/en/blog/tips-for-using-templating-in-governance-policies-part-1[Part 1]
  ** link:https://www.redhat.com/en/blog/tips-for-using-templating-in-governance-policies-part-2[Part 2]
  ** link:https://developers.redhat.com/articles/2025/03/19/tips-using-templating-governance-policies-part-3[Part 3]
