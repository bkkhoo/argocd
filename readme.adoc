...
** link:bootstrap/ansible/bootsrtap.yaml[Bootstrap ansible playbook] creates the secrets on global-hub `rhacm-policies` and `openshift-gitops` namespaces. The access tokens are stored in link:bootstrap/ansible/vars/github-token-vault-encrypted.yaml[ansible-vault encrypted file].
