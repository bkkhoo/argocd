...
. Modify link:helm-charts/openshift-gitops[openshift-gitops Helm chart] to deploy operator **__without__** also auto-create argocd instance (set environment variable `DISABLE_DEFAULT_ARGOCD_INSTANCE=true`), then deploy the argocd instance separately.
