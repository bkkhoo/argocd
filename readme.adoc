...
This diagram illustrates how the different components in the Multicluster Global Hub come together to deliver multicluster management at very high scale.  The root operator is the xref:apps/00-global-hub-app-of-apps[global-hub-app-of-apps ArgoCD application], which automatically syncs manifests from GitHub to managed hubs and managed clusters.

image::./images/multicluster-global-hub.png[Multiluster Global Hub]
...
The commands above deploy the following components on the global Hub cluster:
...
# dev environment
oc apply -k apps/00-managed-hub-app-of-apps/dev
----
