...
│ │ └── base
│ ├── 00-managed-hub-app-of-apps      # app-of-apps deployment of managed hub
│ │ ├── base
│ │ ├── dev
│ │ ├── prod
│ │ └── uat
│ ├── 01-managed-hub-bootstrap        # apps to bootstrap managed hub, contains:
│ │ ├── dev                           #   - manifests to deploy rhacm and openshift-gitops
│ │ ├── prod                          #   - manifests to deploy rhacm integration with openshift-gitops
│ │ └── uat                           #   - manifests for managed-hub-app-of-apps
...
│ └── ansible
│     └── tasks
├── helm-charts                       # helm charts; the charts are used by
...
** app-of-apps for managed hub (`root-app/managed-hub/<dev|uat|prod>`) to dev/uat/prod managed hubs:
...
- Enable RHACM integration with OpenShift GitOps.
- deploys managed hub app-of-apps (`root-app/managed-hub/<dev|uat|prod>`) to dev/uat/prod managed hubs; the app-of-apps, in turn deploys:
** RHACM policies (`apps/rhacm-policies/base`) to `local-cluster`.
