...
[source,bash]
----
ARGOCD_SERVER="$(oc -n openshift-gitops get route openshift-gitops-server -o jsonpath={'.spec.host}')"
ARGOCD_TOKEN="$(argocd account generate-token --account api-admin --expires-in 12h)"

curl -k -H "Authorization: Bearer ${ARGOCD_TOKEN}" https://${ARGOCD_SERVER}/api/v1/clusters | jq
----
